...
export async function prepareTherapist(exampleLogin, therapistBody) {
  ...
  therapistBody.user = await loginUserAndGetUser(
    exampleLogin.email,
    exampleLogin.password
  );
...
export async function preparePatient(exampleLogin, patientBody) {
  await registerTestUser(exampleLogin.email, exampleLogin.password);
  patientBody.patient = await User.findOne({ email: exampleLogin.email });
  patientBody.userCookie = await loginUserAndGetCookie(
    exampleLogin.email,
    exampleLogin.password
  );
  patientBody.user = await loginUserAndGetUser(
    exampleLogin.email,
    exampleLogin.password
  );
}
